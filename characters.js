...
const els = {
  tiers: document.querySelector("#tiers"), template: document.querySelector("#cardTemplate"), search: document.querySelector("#searchInput"), sort: document.querySelector("#sortSelect"),
...
function saveView() { localStorage.setItem(KEYS.view, JSON.stringify({ shows: els.shows.filter((b) => b.checked).map((b) => b.value), roles: els.roles.filter((b) => b.checked).map((b) => b.value), search: els.search.value, sort: els.sort.value })); }
function restoreView() { const view = readJson(KEYS.view, null); if (!view) return; els.shows.forEach((box) => { box.checked = Array.isArray(view.shows) && view.shows.includes(box.value); }); els.roles.forEach((box) => { box.checked = Array.isArray(view.roles) && view.roles.includes(box.value); }); els.search.value = typeof view.search === "string" ? view.search : ""; els.sort.value = ["relevance", "az", "za"].includes(view.sort) ? view.sort : "relevance"; }
...
els.search.addEventListener("input", () => { saveView(); render(); }); els.sort.addEventListener("change", () => { saveView(); render(); }); [...els.shows, ...els.roles].forEach((box) => box.addEventListener("change", () => { saveView(); render(); }));
